...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8746039437:AAEOkl9Jo400konfMtwg65d-KqCPAtnDNec"   # ← ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("8086581937", "0"))
...
BRAND       = os.environ.get("BRAND_NAME", "Rev Hosting Bot")
BRAND_VER   = os.environ.get("BRAND_VERSION", "v1.0")
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = os.environ.get("SUPPORT_USERNAME", "@azurethena")
UPDATE_CH   = os.environ.get("https://t.me/ddosrevv", "")
...
def _build_local_photos() -> None:
    ...
    print(f"[photos] banner font: {font_path or 'NONE found -> plain-letter fallback (deploy the fonts/ folder)'}", flush=True)

    def _hex(c: str) -> Tuple[int, int, int]:
        c = c.lstrip("#")
        return int(c[0:2], 16), int(c[2:4], 16), int(c[4:6], 16)

    # Without any TrueType font Pillow's built-in face has no small-caps
    # glyphs (they render as boxes), so fall back to plain letters.
    _SC_MAP = str.maketrans(
        "\u1d00\u0299\u1d04\u1d05\u1d07\ua730\u0262\u029c\u026a\u1d0a\u1d0b\u029f\u1d0d\u0274\u1d0f\u1d18\u01eb\u0280\ua731\u1d1b\u1d1c\u1d20\u1d21\u028f\u1d22",
        "ABCDEFGHIJKLMNOPQRSTUVWYZ")
    for key, (text, color, sub) in _PHOTO_SPECS.items():
        if not font_path:
            text, sub = text.translate(_SC_MAP), sub.translate(_SC_MAP)
        ...
        if custom_out.exists() and custom_out.stat().st_size > 1024:
            ...
            continue
        out = out_dir / f"{key}.v2.png"   # v2: old cached banners had missing-glyph boxes
...
def _wrap_quote_bold(text):
    ...
    if s.startswith(_QUOTE_OPEN):
        return s
    # Telegram does not allow a blockquote nested inside another one (the
    # footer helpers emit their own <blockquote>), so flatten inner ones.
...
def safe_env(bot_dir: Path, extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    # Restricted PATH for hosted bots, plus the directories of node/npm as
    # installed on this host (Nix/Railway puts them outside /usr/bin).
    _path_dirs = ["/usr/local/bin", "/usr/bin", "/bin"]
    for _tool in ("node", "npm"):
        _p = shutil.which(_tool)
        if _p:
            _d = os.path.dirname(_p)
            if _d not in _path_dirs:
                _path_dirs.append(_d)
    env["PATH"]    = ":".join(_path_dirs)
...
def start_child(b: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if kind == "node":
        ...
        if not node_bin:
            return {"ok": False, "error": "Node.js is not installed on this server (add nodejs to the build, see nixpacks.toml)."}
        cmd = [node_bin, entry]
    else:
    ...
    def _wipe_source_files(bot_path: Path, wait_sec: float = 6.0) -> None:
        ...
        if not bot_path.is_dir():      # bot was deleted during the grace period
            return
        for _f in list(bot_path.iterdir()):
            try:
...
# (Actual list is loaded from settings via _load_required_groups(); see below.)
...
def cb_verify(call: types.CallbackQuery) -> None:
    ...
    try:
        ...
        # Session lost (panel restarted / janitor cleanup): silently start a
        # fresh one instead of telling the user to /start again.
        if not state:
            _ack("Session expired - sending a new captcha")
            try:
                bot.delete_message(chat_id, call.message.message_id)
            except Exception:
                pass
            require_verified(chat_id, uid)
            return

        # A click on an older captcha message that is no longer the active one.
        if state.get("msg_id") and call.message.message_id != state["msg_id"]:
            _ack("That captcha is outdated - use the newest one")
            try:
                bot.delete_message(chat_id, call.message.message_id)
            except Exception:
                pass
        ...
        # NEW captcha (regen)
        if data == "new":
            if state.get("regens", 0) >= 5:
                _ack("Too many regenerations.")
                return
            _ack("New captcha")
            try:
                bot.delete_message(chat_id, call.message.message_id)
                ...
                pass
            with _verify_lock:
                cur = VERIFY_STATES.get(uid)
                if cur is not None:
                    cur["regens"] = cur.get("regens", 0) + 1
        ...
        if data == state["answer"]:
            ...
            get_or_create_user(call.from_user)     # make sure the record exists
            _mark_verified(uid)
            _ack("Verified")
            try:
                bot.delete_message(chat_id, call.message.message_id)
                ...
                pass
            try:
                audit(uid, "captcha_pass", f"verified after {state.get('tries', 0)} try(s)")
            except Exception:
                pass
            # Honour the optional "join our groups" requirement too.
            if not require_group_membership(chat_id, uid):
                return
            intro = (
        ...
        if tries >= 3:
            with _verify_lock:
                VERIFY_STATES.pop(uid, None)
            _ack("Wrong 3 times - new captcha.")
            try:
                bot.delete_message(chat_id, call.message.message_id)
            except Exception:
                pass
            ...
            _ack(f"Wrong character. {3 - tries} try(s) left.")
    except Exception as e:
        print(f"[verify] cb_verify error: {e}", flush=True)
        traceback.print_exc()
    finally:
        _ack("")     # never leave the button spinning
...
def _admin_subroute_legacy(call: types.CallbackQuery, data: str) -> None:
    if data == "adm_stats":
...
def action_adm_force_scan_all(call: types.CallbackQuery) -> None:
    ...
    def _bg() -> None:
        ...
        for bid, info in pending[:5]:
            ...
            try:
                files_added = []
                for _f in list(b["enc_files"])[:3]:
                    _k = KEYRING.fetch(_f["key_id"])
                    if _k:
                        files_added.append((_f.get("rel_path") or _f.get("filename", "file"),
                                            read_encrypted(Path(_f["enc_path"]), _k)))
...
def action_adm_gh_run_file(call: types.CallbackQuery, repo: str, path: str) -> None:
    ...
    def _bg() -> None:
        try:
            ...
            _meta = store_uploaded_file(call.from_user, fname, raw)
            enc_files: List[Dict[str, Any]] = [{
                "key_id": _meta["key_id"], "enc_path": _meta["path"],
                "filename": fname, "rel_path": fname,
            }]
...
def on_document(m: types.Message) -> None:
    ...
    if st.get("flow") in ("await_import_db", "await_adm_import_cfg"):
        if not is_owner(uid):
        ...
        try:
            ...
            if st.get("flow") == "await_import_db":
                ...
                audit(uid, "import_db", f"size={len(raw)} ok={ok}")
                bot.reply_to(m, f"{G['ok'] if ok else G['no']} {esc(msg)}")
            else:
                payload = json.loads(raw.decode("utf-8"))
                cfg = payload.get("settings", payload) if isinstance(payload, dict) else None
                if not isinstance(cfg, dict):
                    bot.reply_to(m, f"{G['no']} Invalid settings file."); return
                settings_save(cfg)
                audit(uid, "import_cfg", f"keys={len(cfg)}")
                bot.reply_to(m, f"{G['ok']} Imported {len(cfg)} settings.")
...
def on_photo(m: types.Message) -> None:
    ...
    if st.get("flow") in ("await_admin_photo", "await_menu_photo") and (is_owner(uid) or admin_can(uid, "manage_admins") or is_admin(uid)):
    ...
    try:
        if flow == "await_gh_repo_url":
            ...
            if not is_admin(uid) and not _user_can_host_gh(u_doc):
                USER_STATES.pop(uid, None)
                bot.reply_to(m, f"{G['no']} GitHub hosting is not available on your plan."); return
            repo_url = (text or "").strip()
            if not re.match(r"^https://github\.com/[A-Za-z0-9_.\-]+/[A-Za-z0-9_.\-]+?(\.git)?/?$", repo_url):
                bot.reply_to(m, f"{G['no']} Invalid URL \u2014 use https://github.com/owner/repo"); return
            USER_STATES.pop(uid, None)
            bot.reply_to(m, f"\u23f3 Cloning repo\u2026 This may take a minute.")
            def _bg_clone():
                d2 = db_load()
                u_doc2 = d2["users"].get(str(uid), {})
                user_token = _dec_secret(u_doc2.get("gh_user_token"))
                ...
                if not res.get("ok"):
                    ...
                    return
                # Cloned code must pass the same security scan as uploads.
                try:
                    _scan_files = []
                    for _p in list(bot_dir.rglob("*"))[:2000]:
                        if _p.is_file() and _p.suffix in (".py", ".js") and _p.stat().st_size < 1_000_000:
                            _scan_files.append((str(_p.relative_to(bot_dir)), _p.read_bytes()))
                            if len(_scan_files) >= 10: break
                    _scan = _run_security_scan(_scan_files, uploader_uid=uid)
                except Exception:
                    _scan = {"recommendation": "MANUAL_REVIEW", "summary": "scan failed"}
                if _scan.get("recommendation") != "APPROVE" and not is_admin(uid):
                    shutil.rmtree(str(bot_dir), ignore_errors=True)
                    try:
                        bot.send_message(uid, f"<b>{G['no']} Repo rejected by security scan</b>\n"
                                              f"<code>{esc(str(_scan.get('summary',''))[:300])}</code>", parse_mode="HTML")
                    except Exception: pass
                    return
        ...
        if flow == "await_gh_user_token":
            ...
            USER_STATES.pop(uid, None)
            d = db_load()
            d["users"].setdefault(str(uid), {})["gh_user_token"] = _enc_secret(tok)
            db_save(d)
...
def _handle_env_kv(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    if key in SECRET_ENV_NAMES or key.upper() in SECRET_ENV_NAMES:
        bot.reply_to(m, f"{G['no']} {sc('That env name is protected')}."); return
    b = find_bot(st["bot_id"])
    if not b:
        bot.reply_to(m, f"{G['no']} {sc('Bot not found')}."); return
    if b.get("owner") != m.from_user.id and not is_admin(m.from_user.id):
        bot.reply_to(m, f"{G['no']} {sc('Not yours')}."); return
    env = b.get("env") or {}
    env[key] = value
    b["env"] = env
...
def _ff_get(key):
    # Single storage format: one "ff_<key>" setting per flag (this is what
    # the admin toggle buttons write). Legacy "feature_flags" dict is read
    # only as a fallback so old data isn't lost.
    v = get_setting(f"ff_{key}", None)
    if v is None:
        legacy = get_setting("feature_flags", {}) or {}
        v = legacy.get(key, _FEATURE_FLAG_DEFAULTS.get(key, True))
    return bool(v)


def _ff_set(key, val):
    set_setting(f"ff_{key}", bool(val))
...
def _ff_reset_all():
    for _k, _v in _FEATURE_FLAG_DEFAULTS.items():
        set_setting(f"ff_{_k}", bool(_v))
    set_setting("feature_flags", {})
...
def _register_extra_routes(data, call):
    ...
    if data == "adm_import_db":
        if not is_owner(call.from_user.id):
            ack(call, "Owner only"); return True
        USER_STATES[call.from_user.id] = {"flow": "await_import_db"}
        ack(call)
        bot.send_message(call.message.chat.id,
                         f"{G['upload']} {sc('Send the exported full-DB JSON file. This REPLACES the current database')}.")
        return True
    return False
...
# ─── Small secret-at-rest helpers (per-user GitHub tokens) ──────────────
def _secret_fernet() -> Fernet:
    return Fernet(base64.urlsafe_b64encode(hashlib.sha256(f"secret-v1|{TOKEN}".encode()).digest()))


def _enc_secret(plain: str) -> str:
    return "enc:" + _secret_fernet().encrypt(plain.encode()).decode()


def _dec_secret(val: Optional[str]) -> Optional[str]:
    if not val:
        return None
    if not val.startswith("enc:"):
        return val   # legacy plaintext value
    try:
        return _secret_fernet().decrypt(val[4:].encode()).decode()
    except Exception:
        return None
...
def _captcha_font(size: int):
    ...
    for fp in _CAPTCHA_FONT_PATHS:
        try:
            ...
            continue
    # No system TTF found (common on minimal containers). Pillow's classic
    # load_default() bitmap font is ~10px regardless of `size`, which made
    # captcha characters nearly invisible next to the 90px red circle.
    # Pillow >=10 supports a scalable default via load_default(size=...).
    try:
...
def _gen_captcha_image() -> Tuple[Optional[bytes], str, List[str]]:
    ...
    # Guard against Pillow's legacy load_default() bitmap font, which stays
    # ~10px tall no matter what size we ask for (see _captcha_font). Drawing
    # that inside a 90px red circle produced a captcha where only the circle
    # was visible — the exact "empty captcha" symptom this fixes. If the
    # rendered glyph height is too small to be legible, skip the image
    # entirely and fall back to the text-only captcha below.
    if font is not None:
        try:
            probe = ImageDraw.Draw(Image.new("RGB", (10, 10)))
            bbox = probe.textbbox((0, 0), "A", font=font)
...
def _send_captcha(chat_id: int, uid: int) -> None:
    """Send a captcha and register the session. Never leaves the user
    without a working captcha: image -> plain-caption image -> text-only
    -> (last resort) a clear error and a clean session so /start retries."""
    ...
    plain_cap = "Human verification\nOne character has a red circle around it.\nTap that exact character below."
    sent_id: Optional[int] = None
    if png is not None:
        for attempt in (("html", cap), ("plain", plain_cap)):
            try:
                kw = {"parse_mode": "HTML"} if attempt[0] == "html" else {"parse_mode": ""}
                m2 = bot.send_photo(chat_id, png, caption=attempt[1], reply_markup=kb, **kw)
                sent_id = m2.message_id
                break
            except Exception as e:
                print(f"[verify] send_photo ({attempt[0]}) failed: {e}", flush=True)
    if sent_id is None:
        # Text-only degraded mode (no usable font / photo upload blocked).
        try:
            m2 = bot.send_message(
                chat_id,
                f"<b>{G['shield']} {sc('Human verification')}</b>\n"
                f"{sc('Tap this character')}: <b><code>{esc(correct)}</code></b>",
                parse_mode="HTML", reply_markup=kb)
            sent_id = m2.message_id
        except Exception as e:
            print(f"[verify] text captcha failed: {e}", flush=True)
    if sent_id is None:
        with _verify_lock:
            VERIFY_STATES.pop(uid, None)      # so the next /start starts clean
        try:
            bot.send_message(chat_id, f"{G['no']} {sc('Verification is temporarily unavailable, please send /start again')}.")
        except Exception:
            pass
        return
    with _verify_lock:
        ...
        VERIFY_STATES[uid] = {
            ...
            "chat_id": chat_id, "tries": 0, "regens": int(prev.get("regens", 0)),
            "ts": time.time(),
        }
...
def require_verified(chat_id: int, uid: int) -> bool:
    """Returns True once the user is verified. Otherwise makes sure the
    user always gets *some* response — never silently drops /start,
    which previously left users stuck after tapping /start a second
    time while a captcha was already pending."""
    if _is_verified(uid):
        return True
    with _verify_lock:
        st = VERIFY_STATES.get(uid)
        now = time.time()
        # Progress-bar animation is still running (first few seconds) —
        # a second /start here would just stack a duplicate animation,
        # so stay quiet for that short window only.
        if st and not st.get("msg_id") and now - st.get("ts", 0) < 6:
            return False
        if st and st.get("msg_id"):
            # A captcha is already on screen. Instead of going silent,
            # nudge the user toward it (rate-limited so spam-tapping
            # /start doesn't flood them with reminders).
            ...
            if now - last_nudge >= 5:
                ...
                try:
                    bot.send_message(
                        chat_id,
                        f"{G['shield']} {sc('You already have a captcha waiting above — solve it, or tap New captcha')}.",
                    )
                except Exception:
                    pass
            return False
        # No session, or a stale/broken one (e.g. the progress-bar thread
        # died before ever sending the captcha) — start fresh.
        VERIFY_STATES[uid] = {
...
def render_admin_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    # Fall back to the full legacy dispatcher (feature flags, language,
    # webhook, subscriptions, ...), then to the extra-route table.
    try:
        _admin_subroute_legacy(call, data)
        return
    except _NoRoute:
        pass
...
def main() -> int:
    ...
    threading.Thread(target=_sched_check_and_run, daemon=True, name="scheduler").start()
